#!/usr/bin/env python3
"""Benchmark minimal ROAS-v2 at exactly 20Q."""
...
from oes.quantum.active_space_roas_v2 import build_helium_roas_v2_20q
...
def main():
    ...
    mol, mf, C_active, receipt = build_helium_roas_v2_20q(
        source_basis=d_aug,
    ...
    if abs(e0 - e_ref) >= 1e-9:
        raise RuntimeError(f"ROAS-v2 20Q OES/PySCF FCI mismatch: {e0-e_ref} Ha")
    ...
    if not triplets or not singlets:
        raise RuntimeError("ROAS-v2 did not resolve both spin sectors")
    ...
    if not dark or not bright:
        raise RuntimeError("ROAS-v2 lost dark or bright singlet class")
