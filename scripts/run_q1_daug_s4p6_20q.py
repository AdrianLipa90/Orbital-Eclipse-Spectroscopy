#!/usr/bin/env python3
"""Benchmark symmetry-complete 4s + 3p + 3(r2 p) at exactly 20Q."""
...
def main():
    ...
    if abs(e0 - e_ref) >= 1e-9:
        raise RuntimeError(f"s4+p6 20Q OES/PySCF FCI mismatch: {e0-e_ref} Ha")
    ...
    if not triplets or not singlets:
        raise RuntimeError("s4+p6 20Q did not resolve both spin sectors")
    ...
    if not dark or not bright:
        raise RuntimeError("s4+p6 20Q lost dark or bright singlet class")
