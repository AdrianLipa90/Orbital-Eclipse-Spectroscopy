#!/usr/bin/env python3
"""Benchmark the state-balanced d-aug hybrid selector at exactly 20 qubits."""
...
from oes.quantum.active_space_hybrid import build_helium_hybrid_20q
...
def main():
    ...
    mol, mf, C_active, receipt = build_helium_hybrid_20q(
        source_basis=d_aug,
    ...
    if abs(e0 - e_ref) >= 1e-9:
        raise RuntimeError(f"d-aug hybrid 20Q OES/PySCF FCI mismatch: {e0-e_ref} Ha")

    s2mat = spin_squared_matrix(10, basis)
    states = classify_states(evals, evecs, s2mat, limit=min(160, len(evals)))
    ...
    if not triplets or not singlets:
        raise RuntimeError("d-aug hybrid 20Q did not resolve both spin sectors")
    ...
    if not dark or not bright:
        raise RuntimeError("d-aug hybrid 20Q lost dark or bright singlet class")
