#!/usr/bin/env python3
"""Predict He I from a d-aug source compressed to exactly 20 qubits.

Active-space selection uses only the calculated source Hamiltonian: one ground
natural orbital, TDA/NTO bright directions, and low-energy canonical fill.  NIST
values are read only after the active space is frozen, for benchmark residuals.
"""
...
from oes.quantum.active_space_tda import build_helium_taas_v2
...
def main():
    ...
    mol, mf, C_active, receipt = build_helium_taas_v2(
        source_basis=d_aug,
    ...
    if abs(e0 - e_ref) >= 1e-9:
        raise RuntimeError(f"d-aug 20Q OES/PySCF FCI mismatch: {e0-e_ref} Ha")

    s2mat = spin_squared_matrix(10, basis)
    states = classify_states(evals, evecs, s2mat, limit=min(140, len(evals)))
    ...
    if not triplets or not singlets:
        raise RuntimeError("d-aug 20Q did not resolve both triplet and singlet sectors")
    ...
    dark = [row for row in singlet_rows if row["f"] < 1e-6]
    bright = [row for row in singlet_rows if row["f"] > 1e-5]
    if not dark or not bright:
        raise RuntimeError("d-aug 20Q lost dark or bright singlet class")
    ...
    payload = {
        ...
        "protocol": "D-AUG-TAAS-TDA-NTO-20Q",
