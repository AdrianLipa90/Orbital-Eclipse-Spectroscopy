#!/usr/bin/env python3
"""Evaluate TAAS-v3 TDA/NTO compression at the unchanged 20Q budget."""
...
def main():
    ...
    # Externally name this candidate TAAS-v3 to keep the pre-existing TAAS-v2
    # operator-response-NO protocol immutable for comparison.
    receipt_dict = receipt.as_dict()
    receipt_dict["protocol"] = "TAAS-v3-TDA-NTO"
    ...
    if abs(e0 - e_ref) >= 1e-9:
        raise RuntimeError(f"TAAS-v3 OES/PySCF active FCI mismatch: {e0-e_ref} Ha")

    s2mat = spin_squared_matrix(10, basis)
    states = classify_states(evals, evecs, s2mat, limit=min(120, len(evals)))
    ...
    if not triplets or not singlets:
        raise RuntimeError("TAAS-v3 did not resolve both triplet and singlet excited sectors")
    triplet, singlet = triplets[0], singlets[0]
    ...
    if not bright_rows:
        raise RuntimeError("TAAS-v3 found no E1-bright singlet")
    bright, bright_f, bright_mu = min(bright_rows, key=lambda item: item[0].excitation_ev)

    calibration_rms = (
        ((triplet.excitation_ev - TARGETS["1s2s_3S1"]) ** 2 +
         (singlet.excitation_ev - TARGETS["1s2s_1S0"]) ** 2) / 2.0
    ) ** 0.5

    payload = {
        "backend": "SIMULATED_REFERENCE",
        "compression": receipt_dict,
        "n_spin_orbitals": 20,
