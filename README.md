...
## Montgomery–Dyson phase spectroscopy — forced core / candidate OES binding

A downstream transition-spectrum diagnostic is available in oes/phase_spectroscopy.py, with the mathematical and claim-boundary contract in formalism/MONTGOMERY_DYSON_PHASE_SPECTROSCOPY_V0_1.md. It maps unfolded transition coordinates to unwrapped phase, exposes the GUE pair-correlation reference, and includes prime-power phase-scramble reverse controls. It does not modify the OES physical solvers or assert that atomic/molecular spectra are generated by prime powers. Core phase-correlation law is DERIVED_IN_FRAMEWORK / FORCED_PREDICTION via the Hardy–CAR theorem; OES physical binding remains CANDIDATE and requires independent repository/domain validation.
