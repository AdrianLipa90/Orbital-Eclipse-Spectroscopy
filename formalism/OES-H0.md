...
The reference relativistic layer evaluates the hydrogenic Dirac energy by `(n,j)` using the ordinary-hydrogen reduced mass as the compact recoil approximation. The `np_1/2` / `np_3/2` series for `n=2..5` is benchmarked against locked NIST values and remains inside the declared `0.5%` per-level tolerance without per-level refitting.
...
For the `n=2` Lamb interval, OES-H0 evaluates a controlled leading low-Z approximation with
...
For ordinary hydrogen this layer gives a `2S_1/2-2P_1/2` interval of about `1050.55 MHz`, compared with the locked NIST target near `1057.85 MHz`, satisfying the declared `1%` leading-model tolerance. The remaining `~7.30 MHz` is retained for higher-order self-energy, recoil, two-loop, finite-size and related QED contributions.
...
- analytic gross-color reconstruction — PASS,
- orbital E1 flavor gates — PASS,
- central exposure kernels — PASS,
- radial cancellation/coherence — PASS,
- reduced-mass Dirac fine-structure reference — PASS for locked `np`, `n=2..5` series,
- linear Zeeman and p-state Paschen–Back continuation — PASS,
- leading one-loop Lamb/QED reference — PASS at the declared `1%` tolerance,
