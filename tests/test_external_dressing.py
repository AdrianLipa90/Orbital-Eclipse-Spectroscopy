...
class ExternalDressingTests(unittest.TestCase):
    ...
    def test_en2_degenerate_block_is_invariant_under_active_rotation(self):
        external = ExternalCouplingSpace(
            external_basis=(4, 8),
            diagonal_hartree=np.array([-0.5, 0.0], dtype=float),
            coupling_qp=np.array([[0.2, 0.1], [0.0, 0.3]], dtype=float),
            n_full_spin_orbitals=4,
            n_active_spin_orbitals=2,
        )
        identity_states = np.eye(2)
        theta = 0.37
        rotation = np.array(
            [[np.cos(theta), -np.sin(theta)], [np.sin(theta), np.cos(theta)]],
            dtype=float,
        )
        a = en2_degenerate_block(-1.0, identity_states, external)
        b = en2_degenerate_block(-1.0, rotation, external)
        self.assertTrue(
            np.allclose(
                a["correction_eigenvalues_hartree"],
                b["correction_eigenvalues_hartree"],
                atol=1e-14,
            )
        )
        self.assertAlmostEqual(
            a["mean_correction_hartree"],
            b["mean_correction_hartree"],
            places=14,
        )
