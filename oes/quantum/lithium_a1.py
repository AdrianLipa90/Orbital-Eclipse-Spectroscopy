...
def prepare_lithium_integrals(basis_name: str = "cc-pVTZ", n_spatial: int = 10):
    """Generate ROHF molecular-orbital integrals for isolated neutral lithium."""
    try:
    ...
    mol = gto.M(
        ...
        unit="Bohr",
        charge=0,
        spin=1,  # 2S ground state: N_alpha-N_beta = 1
        verbose=0,
    )
    mf = scf.ROHF(mol)
    mf.conv_tol = 1e-12
    ...
    coeff = np.asarray(mf.mo_coeff[:, :n_spatial], dtype=float)
    ...
    return mol, h1, eri, dip, rohf_energy
...
def run_lithium_a1(basis_name: str = "cc-pVTZ", n_spatial: int = 10) -> LithiumA1Result:
    ...
    mol, h1, eri, dip, rohf_energy = prepare_lithium_integrals(
    ...
    return LithiumA1Result(
        ...
        first_bright_excitation_ev=float(np.mean([row[1] for row in manifold])),
